...
- Depois em `Role Mappings` e no combo `Client-Roles` escolha `Logistica` em `Avalible Roles` selecione as permissões(logistica) e clique em `Add Selected` (Adicionando permissões ao Realm (Reino) onde o usuário vai acessar para obter o token).
